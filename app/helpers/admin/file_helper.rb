module Admin::FileHelper
  ...
  def save_for(asset)
    "<a id='save-#{asset.id}' href='#' style='display: none'>save</a><a id='cancel-#{asset.id}' style='display: none'>cancel</a>"
  end
  ...
  def link_to_rename_file(asset)
    # link = link_to_remote asset.basename, :url => {:controller => "admin/file", :action => "edit_file_name", :id => asset.id, :v => asset.lock}, :html => {:method => :post} #, :update => "page-#{asset.id}"
    # "<a href='/admin/files/edit?id=#{asset.id}&v=#{asset.lock}'>#{asset.basename}</a>"
    (asset.respond_to?(:root?) and asset.root?) ? asset.basename : "<a url='/admin/files/edit?id=#{asset.id}&v=#{asset.lock}'>#{asset.basename}</a>"
  end
